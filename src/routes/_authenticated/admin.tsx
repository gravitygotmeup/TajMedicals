...
import { 
  ShieldAlert, Sparkles, TrendingUp, PackageCheck, 
  ShoppingBag, ClipboardList, Plus, Trash2, CheckCircle2,
  IndianRupee, FileImage, ExternalLink, RefreshCw, Pill
} from "lucide-react";
import { sendCustomerPackagingEmail } from "@/lib/email.server";
...
function AdminPage() {
  ...
  const handleProcessingItemPriceChange = (index: number, val: number) => {
    setProcessingItems(processingItems.map((item, i) => 
      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="mt-1 text-emerald-900/70">
              Manage incoming prescription requests, set sterile packaging pricing, and keep track of pickups.
            </p>
          </div>
          <Button variant="outline" size="sm" onClick={() => { fetchOrders(); fetchInventory(); }} className="border-emerald-100 bg-white">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="rounded-2xl border border-emerald-100 bg-white p-5 shadow-lg shadow-emerald-950/5 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-emerald-950/40 uppercase tracking-wider">Total Sales</p>
          ...
          <div className="rounded-2xl border border-emerald-100 bg-white p-5 shadow-lg shadow-emerald-950/5 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-emerald-950/40 uppercase tracking-wider">Pending Review</p>
          ...
          <div className="rounded-2xl border border-emerald-100 bg-white p-5 shadow-lg shadow-emerald-950/5 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-emerald-950/40 uppercase tracking-wider">Awaiting Pickup</p>
          ...
          <div className="rounded-2xl border border-emerald-100 bg-white p-5 shadow-lg shadow-emerald-950/5 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-emerald-950/40 uppercase tracking-wider">Completed Pickups</p>
        ...
        <Tabs defaultValue="queue" className="space-y-6">
          <TabsList className="bg-white border border-emerald-100 rounded-xl p-1 w-full max-w-sm">
            <TabsTrigger value="queue" className="rounded-lg font-bold data-[state=active]:bg-emerald-600 data-[state=active]:text-white">
              Order Queue
            </TabsTrigger>
            <TabsTrigger value="inventory" className="rounded-lg font-bold data-[state=active]:bg-emerald-600 data-[state=active]:text-white">
              Inventory Catalog
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="queue" className="space-y-6 outline-none">
            {loadingOrders ? (
              ...
              <div className="grid xl:grid-cols-12 gap-8 items-start">
                ...
                <div className={`xl:col-span-7 space-y-4 ${activeProcessingOrder ? "hidden xl:block" : ""}`}>
                  {orders.map((order) => (
                    <div 
                      key={order.id} 
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-emerald-50 pb-3 mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded ${
                            ...
                            order.status === "ready_to_pay" ? "bg-orange-100 text-orange-800" :
                      ...
                      <div className="grid md:grid-cols-2 gap-4 items-start mb-4">
                        <div>
                          <p className="text-[10px] font-bold text-emerald-950/30 uppercase tracking-wider">Customer details</p>
                          <p className="text-sm font-bold text-emerald-950">{order.user_email}</p>
                          <p className="text-xs text-emerald-950/70">{order.user_phone || "No phone"}</p>
                          <p className="text-[11px] text-emerald-950/60 mt-1 truncate">{order.user_address || "No address"}</p>
                        </div>
                        <div>
                          <p className="text-[10px] font-bold text-emerald-950/30 uppercase tracking-wider">Items summary</p>
                          <div className="max-h-[80px] overflow-y-auto mt-1">
                            {order.items && order.items.map((item, idx) => (
                              <p key={idx} className="text-xs text-emerald-950 leading-relaxed font-semibold">
                                • {item.name} <span className="text-emerald-900/50">x{item.quantity}</span>
                              </p>
                            ))}
                          </div>
                        </div>
                      ...
                      {order.prescription_url && (
                        <div className="mb-4">
                          ...
                          >
                            <FileImage className="h-4 w-4 text-emerald-600" /> View Prescription <ExternalLink className="h-3.5 w-3.5" />
                          </a>
                        </div>
                      )}
                      ...
                      <div className="flex justify-between items-center border-t border-emerald-50 pt-3">
                        <div className="flex flex-col">
                          <span className="text-sm font-black text-emerald-950">
                            Invoice: {order.total_price > 0 ? `₹${order.total_price.toFixed(2)}` : "Pending"}
                          </span>
                          {order.total_price > 0 && (
                            <span className={`text-[10px] font-bold uppercase mt-0.5 ${order.payment_status === "paid" ? "text-emerald-600" : "text-amber-600"}`}>
                              {order.payment_status === "paid" ? "Paid" : "Unpaid (Collect at Counter)"}
                            </span>
                          )}
                        </div>
                ...
                {activeProcessingOrder && (
                  <div className="xl:col-span-5 rounded-3xl border-2 border-emerald-600 bg-white p-6 shadow-xl space-y-6">
                    <div className="flex justify-between items-center border-b border-emerald-50 pb-3">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => setActiveProcessingOrder(null)}>Cancel</Button>
                    </div>

                    <div className="text-xs text-emerald-900/60 bg-emerald-50/30 border border-emerald-100/50 rounded-2xl p-4 space-y-2">
                      <p><strong>Customer:</strong> {activeProcessingOrder.user_email}</p>
                      <p><strong>Address:</strong> {activeProcessingOrder.user_address || "None specified"}</p>
                      <p><strong>Customer Notes:</strong> {activeProcessingOrder.notes || "None provided"}</p>
                    </div>
                    ...
                    <div className="border border-emerald-100 rounded-2xl p-4 bg-emerald-50/20 space-y-3">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-950/60">Add Packaged Item</h4>
                      <div className="flex gap-2">
                        <Input 
                    ...
                    <div className="space-y-4">
                      <h3 className="text-xs font-bold uppercase tracking-wider text-emerald-950/40">Assign Packaging Prices</h3>
                      <div className="space-y-3">
                        {processingItems.map((item, idx) => (
                          <div key={idx} className="flex gap-3 justify-between items-center py-2.5 border-b border-emerald-50">
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-bold text-emerald-950 truncate flex items-center gap-1.5">
                                {item.name}
                                {item.isCustom && <span className="bg-amber-100 border border-amber-200 px-1 py-0.5 rounded text-[8px] font-bold text-amber-700">Custom</span>}
                              </p>
                            </div>
                            ...
                            <div className="flex gap-2 shrink-0 items-center">
                              ...
                              <div className="w-14">
                                <Input 
                                  ...
                                  value={item.quantity} 
                                  onChange={(e) => handleProcessingItemQtyChange(idx, parseInt(e.target.value) || 1)}
                                />
                              </div>
                              {/* Price field */}
                              <div className="relative w-20">
                                <span className="absolute left-2 top-2.5 text-xs text-emerald-950/40">₹</span>
                                <Input 
                                  type="number" 
                                  placeholder="Price" 
                                  step="0.01" 
                                  ...
                                  value={item.price || ""} 
                                  onChange={(e) => handleProcessingItemPriceChange(idx, parseFloat(e.target.value) || 0)}
                                />
                              </div>
                              {/* Delete item button */}
                              <Button 
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="adminNotes" className="text-xs font-bold text-emerald-950">Directions for Customer (Admin Notes)</Label>
                      <Textarea 
                        id="adminNotes" 
                    ...
                    <div className="pt-4 border-t border-emerald-50">
                      <div className="flex justify-between items-center mb-4">
                        <span className="text-sm font-bold text-emerald-950">Subtotal calculation</span>
                        <span className="text-lg font-black text-emerald-700">
                          ₹{processingItems.reduce((acc, item) => acc + ((item.price || 0) * item.quantity), 0).toFixed(2)}
                        </span>
                      </div>
          ...
          <TabsContent value="inventory" className="space-y-6 outline-none">
            ...
            <div className="grid lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-7 rounded-3xl border border-emerald-100 bg-white p-6 shadow-xl shadow-emerald-950/5">
                ...
                {loadingInventory ? (
                  <div className="py-12 text-center text-emerald-900/60 font-semibold">Loading catalog...</div>
                ) : medicines.length === 0 ? (
                  <div className="py-12 text-center text-emerald-900/50">Catalog is empty.</div>
                ) : (
                  <div className="space-y-3.5 max-h-[500px] overflow-y-auto pr-1">
                    {medicines.map((med) => (
                      <div key={med.id} className="flex justify-between items-center p-3.5 rounded-2xl border border-emerald-50 bg-emerald-50/15 hover:bg-emerald-50/30 transition">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-emerald-950/50 mt-0.5">{med.category} • stock: {med.stock}</p>
                            {med.description && <p className="text-[10px] text-emerald-950/40 mt-1 italic">{med.description}</p>}
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-sm font-extrabold text-emerald-950">₹{med.price.toFixed(2)}</span>
                          <Button 
                            size="icon" 
                            variant="ghost" 
              ...
              <div className="lg:col-span-5 rounded-3xl border border-emerald-100 bg-white p-6 shadow-xl shadow-emerald-950/5">
                <h2 className="text-lg font-bold text-emerald-950 border-b border-emerald-50 pb-3 mb-4">Add Medicine</h2>
                
                <form onSubmit={handleAddMedicine} className="space-y-4">
                  <div className="space-y-1.5">
                    <Label htmlFor="medName" className="text-xs font-bold text-emerald-950">Medicine Name</Label>
                    <Input 
                      id="medName" 
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="medDesc" className="text-xs font-bold text-emerald-950">Description</Label>
                    <Textarea 
                      id="medDesc" 
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <Label htmlFor="medPrice" className="text-xs font-bold text-emerald-950">Price (₹)</Label>
                      <div className="relative">
                        <span className="absolute left-2.5 top-2.5 text-xs text-emerald-950/40">₹</span>
                        <Input 
                          id="medPrice" 
                          type="number" 
                          step="0.01" 
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="medStock" className="text-xs font-bold text-emerald-950">Initial Stock</Label>
                      <Input 
                        id="medStock" 
                        type="number" 
                        min="0" 
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="medCat" className="text-xs font-bold text-emerald-950">Category</Label>
                    <select
                      id="medCat"
