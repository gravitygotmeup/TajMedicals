...
import { 
  Pill, Plus, Trash2, FileText, UploadCloud, X, 
...
function RequestMedicinePage() {
  ...
  const handleAddCustomItem = (e: React.FormEvent) => {
    ...
    const existing = selectedItems.find(item => item.name.toLowerCase() === customName.trim().toLowerCase());
    if (existing) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let mockPrescriptionUrl = null;
      if (prescriptionFile) {
        mockPrescriptionUrl = "https://images.unsplash.com/photo-1576091160550-2173dba999ef?auto=format&fit=crop&q=80&w=600";
      }
      ...
      const { data, error } = await supabase.from("orders").insert({
        ...
        prescription_url: mockPrescriptionUrl,
      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-1.5 text-emerald-900/70">
            Upload your doctor's prescription slip, list the medicines you need packaged, and our pharmacist will set it up for pickup.
          </p>
        </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-xl shadow-emerald-950/5">
              ...
              <p className="text-xs text-emerald-900/60 mb-4">
                Please upload a photo of your doctor's slip as a proof to verify prescribed medications.
              </p>
              ...
              {prescriptionPreview ? (
                <div className="relative border border-emerald-100 rounded-2xl bg-emerald-50/20 p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="h-14 w-14 rounded-lg overflow-hidden border border-emerald-100 bg-white flex items-center justify-center">
                      <img src={prescriptionPreview} alt="Prescription" className="h-full w-full object-cover" />
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-emerald-950 truncate max-w-[200px]">{prescriptionFile?.name}</p>
                      <p className="text-[10px] text-emerald-950/50">{(prescriptionFile!.size / 1024 / 1024).toFixed(2)} MB</p>
                    </div>
                  </div>
                  <Button size="icon" variant="ghost" className="h-8 w-8 text-emerald-900/50 hover:text-red-500" onClick={handleRemoveFile}>
                    <X className="h-5 w-5" />
                  </Button>
                </div>
              ) : (
                <label className="border-2 border-dashed border-emerald-100 rounded-2xl p-6 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-emerald-50/30 transition group">
                  ...
                  <span className="text-sm font-semibold text-emerald-950">Click to upload prescription</span>
                  <span className="text-[10px] text-emerald-900/50 mt-0.5">Supports PNG, JPG, PDF up to 5MB</span>
            ...
            <div className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-xl shadow-emerald-950/5">
              ...
              <form onSubmit={handleAddCustomItem} className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="w-full sm:w-28 flex gap-2">
                  ...
                  />
                  <Button type="submit" className="h-11 px-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-semibold transition shrink-0">
                    Add
                  </Button>
                </div>
              </form>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-xl shadow-emerald-950/5 sticky top-24">
              ...
              {selectedItems.length === 0 ? (
                ...
                <div className="space-y-3.5 max-h-[300px] overflow-y-auto pr-1 mb-6">
                  {selectedItems.map((item, index) => (
                    <div key={index} className="flex justify-between items-center py-2 border-b border-emerald-50/50 last:border-b-0 last:bg-transparent">
                      <div>
              ...
              <div className="space-y-1.5 mb-6">
                <Label htmlFor="notes" className="text-xs font-bold text-emerald-950">Special Instructions</Label>
                <Textarea
                  id="notes"
              ...
              <div className="space-y-3.5">
                ...
                <Button asChild variant="ghost" className="w-full text-emerald-800 hover:bg-emerald-50 rounded-xl">
